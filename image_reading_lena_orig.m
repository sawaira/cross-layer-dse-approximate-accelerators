...
pop=50;
 ...
 mae_file  = ['mae', int2str(iter),'_',int2str(pop),'.txt']
%  f_n  = [ int2str(ind_1),'.txt']
%  f_n  = [ int2str(ind_1),'.txt']
%  f_n  = [ int2str(ind_1),'.txt']
 Mult=nsga2(pop,iter);
 save('mult_file','Mult');
...
size_mult_matrix=size(Mult,1);
 for index=1:size_mult_matrix
  ...
  toc;
for ind_1=1:10
   % f_n=['mul8_00' int2str(ind_1)];
   
if(ind_1 ==4)
continue;
end
    f_n  = [ int2str(ind_1),'.jpg'] ;% Create Name   
figure(ind_1);
I=imread(f_n);
J = imnoise(I,'speckle');
...
nVar=9;
  ...
  f_n_psnr  = [ 'psnr_',int2str(ind_1),'_',int2str(iter),'_',int2str(pop),'.txt'] ;% Create Name   
  f_n_ssim  = [ 'ssim_',int2str(ind_1),'_',int2str(iter),'_',int2str(pop),'.txt'] ;% Create Name  
