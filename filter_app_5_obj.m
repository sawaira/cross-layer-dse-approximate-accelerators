...
for index=1:50
MA(index,:)=[area_calc(index) ,mse_calc(index)   mae_calc(index) power_calc(index) delay_calc(index)];
...
for it = 1:MaxIt
    ...
    for k = 1:nCrossover/2
       ...
       [area_calc(k,1),mse_calc(k,1),power_calc(k,1),mae_calc(k,1) delay_calc(k,1)] = calculate_positions(VarMin, VarMax, VarSize_Mul,VarMin_Add, VarMax_Add, VarSize_Add,nVar_Mul,nVar_Add,popc(k,1).Position);
         popc(k, 1).Cost =[area_calc(k,1) mse_calc(k,1) mae_calc(k,1) power_calc(k,1) delay_calc(k,1)]  ;
[area_calc(k,2),mse_calc(k,2),power_calc(k,2),mae_calc(k,2) delay_calc(k,2)] = calculate_positions(VarMin, VarMax, VarSize_Mul,VarMin_Add, VarMax_Add, VarSize_Add,nVar_Mul,nVar_Add,popc(k,2).Position);
  popc(k, 2).Cost =[area_calc(k,2) mse_calc(k,2)  mae_calc(k,2) power_calc(k,2) delay_calc(k,2)]  ;    
    ...
    for k = 1:nMutation
        ...
        popm(k).Cost = [area_calc(k) mse_calc(k) mae_calc(k) power_calc(k) delay_calc(k)];%CostFunction(popm(k).Position);
  ...
  iter_i=1;
    for i=1:5:m
     %   if(Costs(i+1)<500 && Costs(i)<8.8e4 && Costs(i+2)< 4.08e4 )
        ...
        Costs_disp(iter_i,2)=Costs(i+2);
        Costs_disp(iter_i,3)=Costs(i+3);
